...
public class DynamicalAddingActivity extends Activity  {

    Intent intent;
    NetworkThread networkThread2;
    int mq2_data;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intent=getIntent();
        networkThread2=new NetworkThread();
        networkThread2.setApikey(intent.getStringExtra("ApiKey"));
        networkThread2.setDeviceID(intent.getStringExtra("DeviceID"));
        ...
        public void run() {
            ...
            while (true) {
                try {
                    networkThread2.setApikey(intent.getStringExtra("ApiKey"));
                    networkThread2.setDeviceID(intent.getStringExtra("DeviceID"));
                    Log.w("Apikey,DeviceID",""+intent.getStringExtra("ApiKey")+intent.getStringExtra("DeviceID"));
                    //array=networkThread1.Get();
                    networkThread2.Get();
                    mq2_data=Integer.parseInt(networkThread2.getMQ2());
                    addEntry();
                    Log.w("mq2_data",""+mq2_data);
                    Thread.sleep(1000);
                }catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void addEntry() {
        ...
        if (lineData != null) {
           ...
           // lineData.addXValue(count + "");
            ...
            //移到某个位置
            mChart.moveViewToX(lastSet.getEntryCount() - 5);
